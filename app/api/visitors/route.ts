...
export async function GET() {
  const count = await upstash<number>("get", [KEY]);
  return Response.json({ count: typeof count === "number" ? count : START });
}

export async function POST() {
  const current = await upstash<number | null>("get", [KEY]);
  if (current === null) {
  ...
  const next = await upstash<number>("incr", [KEY]);
  return Response.json({ count: typeof next === "number" ? next : START });
}
